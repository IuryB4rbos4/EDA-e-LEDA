...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
		int indexLeft = this.left(position),
				indexRight = this.right(position),
				largest = position;

		if (this.isValidIndex(indexLeft) && this.compare(position, indexLeft) < 0)
			largest = indexLeft;
		if (this.isValidIndex(indexRight) && this.compare(largest, indexRight) < 0)
			largest = indexRight;
		if (largest != position) {
