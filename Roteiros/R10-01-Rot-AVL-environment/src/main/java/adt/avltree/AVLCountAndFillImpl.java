...
public class AVLCountAndFillImpl<T extends Comparable<T>> extends
	...
	private boolean avlIterate(T[] array, int left, int right, int height) {
		...
		if (right > left) {
			...
			if (height == 0) {
				...
				boolean result1 = avlIterate(array, left, middle, height - 1), result2 = avlIterate(array, middle + 1, right, height - 1);
	...
	protected void rebalance(BSTNode<T> node) {
		...
		if (Math.abs(balance) > 1)
			...
			else if (this.calculateBalance((BSTNode<T>) node.getRight()) <= 0) {
